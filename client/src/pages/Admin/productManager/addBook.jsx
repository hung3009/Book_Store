...
const AddBookForm = () => {
  ...
  const [data, setData] = useState({
    ...
    Author: "",
    Price: "",
    Image: "", // Set a default empty string
    ISBN: "",
    Genre: "",
    Publisher: "",
    Publish_Year: "",
  ...
  const onFinish = async () => {
    if (data) {
      ...
      axios
        .post(
          `${process.env.REACT_APP_API_PORT}/addbook`,
          JSON.stringify(data),
          {
            headers: {
  ...
  const handleFileUpload = async (file, callback) => {
    try {
      ...
      setData({ ...data, Image: uploadedImageUrl });
  ...
  const handleValuesChange = () => {
    form
      ...
      .then((values) => {
        ...
        setData({...values,Image: imageUrl});
        
        // setData({ ...values, Image: imageLinks?.thumbnail });
      })
      .catch(() => {
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <Upload action={handleFileUpload} listType="picture-card">
            <div>
              <PlusOutlined />
        ...
        >
          ...
          <p>Image: {data?.Image ? <img src={data.Image} alt="Book Image" /> : "N/A"}</p>
          <p>ISBN: {data?.ISBN || ""}</p>
...
export { AddBookForm, ImportBooks };